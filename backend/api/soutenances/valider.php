...
$stmt = $pdo->prepare("SELECT s.*, CONCAT(e.prenom,' ',e.nom) as etudiant, u.id as encadrant_id, u.email as encadrant_email, u.prenom as encadrant_prenom, u.nom as encadrant_nom
    FROM soutenances s JOIN etudiants e ON s.etudiant_id = e.id JOIN users u ON s.encadrant_id = u.id WHERE s.id = ?");
$stmt->execute([$id]);
$sout = $stmt->fetch();

if ($sout) {
    $msg = $decision === 'validee'
        ? "La soutenance de {$sout['etudiant']} a été validée."
        : "La soutenance de {$sout['etudiant']} a été refusée." . ($motif ? " Motif : $motif" : '');
    ...
    $contenu = $decision === 'validee'
        ? "<p>Bonjour {$sout['encadrant_prenom']},</p><p>La soutenance de <strong>{$sout['etudiant']}</strong> a été <strong style='color:#166534;'>validée</strong> par le chef de département.</p>"
        : "<p>Bonjour {$sout['encadrant_prenom']},</p><p>La soutenance de <strong>{$sout['etudiant']}</strong> a été <strong style='color:#991b1b;'>refusée</strong> par le chef de département." . ($motif ? " <br><strong>Motif :</strong> " . htmlspecialchars($motif) : '') . "</p><p>Merci de vous connecter à la plateforme pour ajuster la planification si nécessaire.</p>";
